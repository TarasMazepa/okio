...
expect interface BufferedSource : Source {
  ...
  /**
   * Returns when the buffer contains at least `byteCount` bytes. Throws an [java.io.EOFException]
  ...
  /**
   * Removes two bytes from this source and returns a big-endian short. ``` Buffer buffer = new
   * Buffer() .writeByte(0x7f) .writeByte(0xff) .writeByte(0x00) .writeByte(0x0f); assertEquals(4,
   * buffer.size());
   *
   * assertEquals(32767, buffer.readShort()); assertEquals(2, buffer.size());
   *
   * assertEquals(15, buffer.readShort()); assertEquals(0, buffer.size()); ```
   */
  fun readShort(): Short

  /**
   * Removes two bytes from this source and returns a little-endian short. ``` Buffer buffer = new
   * Buffer() .writeByte(0xff) .writeByte(0x7f) .writeByte(0x0f) .writeByte(0x00); assertEquals(4,
   * buffer.size());
   *
   * assertEquals(32767, buffer.readShortLe()); assertEquals(2, buffer.size());
   *
   * assertEquals(15, buffer.readShortLe()); assertEquals(0, buffer.size()); ```
   */
  fun readShortLe(): Short

  /**
   * Removes four bytes from this source and returns a big-endian int. ``` Buffer buffer = new
   * Buffer() .writeByte(0x7f) .writeByte(0xff) .writeByte(0xff) .writeByte(0xff) .writeByte(0x00)
   * .writeByte(0x00) .writeByte(0x00) .writeByte(0x0f); assertEquals(8, buffer.size());
   *
   * assertEquals(2147483647, buffer.readInt()); assertEquals(4, buffer.size());
   *
   * assertEquals(15, buffer.readInt()); assertEquals(0, buffer.size()); ```
   */
  fun readInt(): Int

  /**
   * Removes four bytes from this source and returns a little-endian int. ``` Buffer buffer = new
   * Buffer() .writeByte(0xff) .writeByte(0xff) .writeByte(0xff) .writeByte(0x7f) .writeByte(0x0f)
   * .writeByte(0x00) .writeByte(0x00) .writeByte(0x00); assertEquals(8, buffer.size());
   *
   * assertEquals(2147483647, buffer.readIntLe()); assertEquals(4, buffer.size());
   *
   * assertEquals(15, buffer.readIntLe()); assertEquals(0, buffer.size()); ```
   */
  fun readIntLe(): Int

  /**
   * Removes eight bytes from this source and returns a big-endian long. ``` Buffer buffer = new
   * Buffer() .writeByte(0x7f) .writeByte(0xff) .writeByte(0xff) .writeByte(0xff) .writeByte(0xff)
   * .writeByte(0xff) .writeByte(0xff) .writeByte(0xff) .writeByte(0x00) .writeByte(0x00)
   * .writeByte(0x00) .writeByte(0x00) .writeByte(0x00) .writeByte(0x00) .writeByte(0x00)
   * .writeByte(0x0f); assertEquals(16, buffer.size());
   *
   * assertEquals(9223372036854775807L, buffer.readLong()); assertEquals(8, buffer.size());
   *
   * assertEquals(15, buffer.readLong()); assertEquals(0, buffer.size()); ```
   */
  fun readLong(): Long

  /**
   * Removes eight bytes from this source and returns a little-endian long. ``` Buffer buffer = new
   * Buffer() .writeByte(0xff) .writeByte(0xff) .writeByte(0xff) .writeByte(0xff) .writeByte(0xff)
   * .writeByte(0xff) .writeByte(0xff) .writeByte(0x7f) .writeByte(0x0f) .writeByte(0x00)
   * .writeByte(0x00) .writeByte(0x00) .writeByte(0x00) .writeByte(0x00) .writeByte(0x00)
   * .writeByte(0x00); assertEquals(16, buffer.size());
   *
   * assertEquals(9223372036854775807L, buffer.readLongLe()); assertEquals(8, buffer.size());
   *
   * assertEquals(15, buffer.readLongLe()); assertEquals(0, buffer.size()); ```
   */
  fun readLongLe(): Long

  /**
   * Reads a long from this source in signed decimal form (i.e., as a string in base 10 with
   * optional leading '-'). This will iterate until a non-digit character is found. ``` Buffer
   * buffer = new Buffer() .writeUtf8("8675309 -123 00001");
   *
   * assertEquals(8675309L, buffer.readDecimalLong()); assertEquals(' ', buffer.readByte());
   * assertEquals(-123L, buffer.readDecimalLong()); assertEquals(' ', buffer.readByte());
   * assertEquals(1L, buffer.readDecimalLong()); ```
   *
   * @throws NumberFormatException if the found digits do not fit into a `long` or a decimal number
   *     was not present.
   */
   ...
   * iterate until a non-hexadecimal character is found. ``` Buffer buffer = new Buffer()
   * .writeUtf8("ffff CAFEBABE 10");
   *
   * assertEquals(65535L, buffer.readHexadecimalUnsignedLong()); assertEquals(' ',
   * buffer.readByte()); assertEquals(0xcafebabeL, buffer.readHexadecimalUnsignedLong());
   * assertEquals(' ', buffer.readByte()); assertEquals(0x10L,
   ...
   * expected values is known in advance. ``` Options FIELDS = Options.of(
   * ByteString.encodeUtf8("depth="), ByteString.encodeUtf8("height="),
   * ByteString.encodeUtf8("width="));
   *
   * Buffer buffer = new Buffer() .writeUtf8("width=640\n") .writeUtf8("height=480\n");
   *
   * assertEquals(2, buffer.select(FIELDS)); assertEquals(640, buffer.readDecimalLong());
   * assertEquals('\n', buffer.readByte()); assertEquals(1, buffer.select(FIELDS));
   * assertEquals(480, buffer.readDecimalLong()); assertEquals('\n', buffer.readByte()); ```
   */
   ...
   * string if this source is empty. ``` Buffer buffer = new Buffer() .writeUtf8("Uh uh uh!")
   * .writeByte(' ') .writeUtf8("You didn't say the magic word!");
   *
   * assertEquals("Uh uh uh! You didn't say the magic word!", buffer.readUtf8()); assertEquals(0,
   * buffer.size());
   *
   * assertEquals("", buffer.readUtf8()); assertEquals(0, buffer.size()); ```
   */
  fun readUtf8(): String

  /**
   * Removes `byteCount` bytes from this, decodes them as UTF-8, and returns the string. ``` Buffer
   * buffer = new Buffer() .writeUtf8("Uh uh uh!") .writeByte(' ') .writeUtf8("You didn't say the
   * magic word!"); assertEquals(40, buffer.size());
   *
   * assertEquals("Uh uh uh! You ", buffer.readUtf8(14)); assertEquals(26, buffer.size());
   *
   * assertEquals("didn't say the", buffer.readUtf8(14)); assertEquals(12, buffer.size());
   *
   * assertEquals(" magic word!", buffer.readUtf8(12)); assertEquals(0, buffer.size()); ```
   */
  fun readUtf8(byteCount: Long): String

  /**
   * Removes and returns characters up to but not including the next line break. A line break is
   * either `"\n"` or `"\r\n"`; these characters are not included in the result. ``` Buffer buffer =
   * new Buffer() .writeUtf8("I'm a hacker!\n") .writeUtf8("That's what I said: you're a nerd.\n")
   * .writeUtf8("I prefer to be called a hacker!\n"); assertEquals(81, buffer.size());
   *
   * assertEquals("I'm a hacker!", buffer.readUtf8Line()); assertEquals(67, buffer.size());
   *
   * assertEquals("That's what I said: you're a nerd.", buffer.readUtf8Line()); assertEquals(32,
   * buffer.size());
   *
   * assertEquals("I prefer to be called a hacker!", buffer.readUtf8Line()); assertEquals(0,
   * buffer.size());
   *
   * assertEquals(null, buffer.readUtf8Line()); assertEquals(0, buffer.size()); ```
   *
   ...
   * returned once the source is exhausted. Use this for human-generated data, where a trailing line
   * break is optional.
   */
   ...
   * If these characters are absent in the stream, an [java.io.EOFException] is thrown. Use this for
   ...
   * another match: ``` Buffer buffer = new Buffer(); buffer.writeUtf8("12345\r\n");
   *
   * // This will throw! There must be \r\n or \n at the limit or before it.
   * buffer.readUtf8LineStrict(4);
   *
   * // No bytes have been consumed so the caller can retry. assertEquals("12345",
   ...
   * buffer. Returns -1 if the stream is exhausted before the requested byte is found. ``` Buffer
   * buffer = new Buffer(); buffer.writeUtf8("Don't move! He can't see us if we don't move.");
   *
   * byte m = 'm'; assertEquals(6, buffer.indexOf(m)); assertEquals(40, buffer.indexOf(m, 12)); ```
   */
   ...
   * expands the buffer as necessary until `bytes` is found. This reads an unbounded number of bytes
   * into the buffer. Returns -1 if the stream is exhausted before the requested bytes are found.
   * ``` ByteString MOVE = ByteString.encodeUtf8("move");
   *
   * Buffer buffer = new Buffer(); buffer.writeUtf8("Don't move! He can't see us if we don't
   * move.");
   *
   * assertEquals(6, buffer.indexOf(MOVE)); assertEquals(40, buffer.indexOf(MOVE, 12)); ```
   */
   ...
   * before the requested byte is found. ``` ByteString ANY_VOWEL =
   * ByteString.encodeUtf8("AEOIUaeoiu");
   *
   * Buffer buffer = new Buffer(); buffer.writeUtf8("Dr. Alan Grant");
   *
   * assertEquals(4, buffer.indexOfElement(ANY_VOWEL)); // 'A' in 'Alan'. assertEquals(11,
   * buffer.indexOfElement(ANY_VOWEL, 9)); // 'a' in 'Grant'. ```
   */
   ...
   * before enough bytes could determine a match. ``` ByteString simonSays =
   * ByteString.encodeUtf8("Simon says:");
   *
   ...
   *
   * ``` val buffer = Buffer() buffer.writeUtf8("abcdefghi")
   *
   * buffer.readUtf8(3) // returns "abc", buffer contains "defghi"
   *
   * val peek = buffer.peek() peek.readUtf8(3) // returns "def", buffer contains "defghi"
