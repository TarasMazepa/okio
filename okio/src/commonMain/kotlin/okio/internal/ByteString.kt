...
internal inline fun ByteString.commonRangeEquals(
  ...
  return (offset >= 0 && offset <= data.size - byteCount && otherOffset >= 0 &&
...
@Suppress("NOTHING_TO_INLINE")
internal inline fun ByteString.commonLastIndexOf(other: ByteString, fromIndex: Int) =
...
internal inline fun ByteString.commonHashCode(): Int {
  ...
  return data.contentHashCode().also { hashCode = it }
}
@Suppress("NOTHING_TO_INLINE")
...
internal inline fun ByteString.commonToString(): String {
  ...
  val text = utf8()
  val safeText =
      text.substring(0, i).replace("\\", "\\\\").replace("\n", "\\n").replace("\r", "\\r")
